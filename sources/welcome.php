...
function PageMain() {
	...
	//generate json for cloud
	$query = 'SELECT keyword, count from keywords WHERE keyword !="Search..." ORDER BY 2 DESC LIMIT 0,15';
	$result = mysqli_query($db, $query);
	$rows = array();
	while($r = mysqli_fetch_assoc($result)) {
		$r['url'] = "./index.php?a=search&q=".$r['keyword'];
	    $rows[] = $r;
	}
	$keywords = str_replace("keyword", "text", json_encode($rows));
	$TMPL['keyword'] = str_replace("count", "size", $keywords);
	//
